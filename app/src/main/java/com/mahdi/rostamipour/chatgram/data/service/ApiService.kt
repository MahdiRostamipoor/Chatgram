...
class ApiService {
    ...
    suspend fun registerUser(name : String , bio : String) : String{
        return httpClient.post("http://10.197.23.222:3000/register"){
    ...
    suspend fun getUsers(userId : Int) : List<User>{
        return httpClient.get("http://10.197.23.222:3000/users"){
            url {
    ...
    suspend fun getMessages(senderId : Int , getterId : Int) : List<GetMessage>{
        return httpClient.get("http://10.197.23.222:3000/getmessages"){
            url{
    ...
    suspend fun sendMessage(sendMessage: SendMessage) : GetMessage{
        return httpClient.post("http://10.197.23.222:3000/sendmessage"){
            contentType(ContentType.Application.Json)
            setBody(sendMessage)
        }.body()
    }
    ...
    suspend fun connectSocket(userId : Int) {
        httpClient.webSocket("ws://10.197.23.222:3000") {
            socketSession = this
